...
export default function Settings() {
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    ...
    outputPrefs: true,
    metadata: true,
    mcpControl: true, // Always expanded by default to make MCP toggle visible
  });
    ...
    <div className="text-white">
      ...
      <div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium">MCP Services</p>
              <p className="text-xs text-gray-400">
          ...
          <div className="text-xs text-gray-400 bg-neutral-800 p-2 rounded">
            <p><strong>Status:</strong> <span className={mcpClientEnabled ? "text-green-400" : "text-red-400"}>{mcpClientEnabled ? "✅ Enabled" : "❌ Disabled"}</span></p>
            <p><strong>Last toggle:</strong> {new Date().toLocaleTimeString()}</p>
            <p><strong>Function:</strong> Controls MCP client integration</p>
          </div>
        </div>
      </div>
    </div>
