#!/usr/bin/env python3
"""
Custom RAG Tool for NAT v1.2.1
Based on the GitHub repository structure and nvidia_rag.py pattern
"""
...
class CustomRAGToolConfig(FunctionBaseConfig, name="custom_rag"):
    """
    Custom RAG tool that integrates with our RAG server using the /generate endpoint
    """
...
@register_function(config_type=CustomRAGToolConfig)
async def custom_rag_tool(config: CustomRAGToolConfig, builder: Builder):
    """
    Custom RAG tool that uses the /generate endpoint with our exact parameters
    """
    import httpx
    ...
    async with httpx.AsyncClient(
        ...
        async def runnable(query: str) -> str:
            try:
                ...
                logger.debug("Sending request to RAG endpoint %s with payload: %s", url, json.dumps(payload, indent=2))
                ...
                logger.exception("Error while running the custom RAG tool", exc_info=True)
