"""
Enhanced RAG Playground with MCP Registry Integration
Provides document search capabilities with advanced MCP server management
"""
...
import numpy as np
import requests
from mcp_client_enhanced import enhanced_mcp_manager, MCP_AVAILABLE
from mcp_registry import mcp_registry, DiscoveryMethod
...
# MCP Registry Routes
@app.route('/mcp/registry/status')
def registry_status():
    """Get MCP registry status"""
    try:
        stats = enhanced_mcp_manager.get_registry_stats()
        return jsonify({
            "status": "success",
            "stats": stats
        })
    except Exception as e:
        logger.error(f"Registry status error: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/mcp/registry/servers')
def registry_servers():
    """Get all registered servers"""
    try:
        servers = enhanced_mcp_manager.get_all_servers_info()
        return jsonify({
            "status": "success",
            "servers": servers
        })
    except Exception as e:
        logger.error(f"Registry servers error: {e}")
        return jsonify({"error": str(e)}), 500
...
def registry_discover():
    ...
    try:
        ...
        discovery_type = data.get('type', 'all')
        
        if discovery_type == 'kubernetes':
            run_async(mcp_registry.discover_kubernetes_services())
        elif discovery_type == 'http':
            urls = data.get('urls', [])
            run_async(mcp_registry.discover_http_services(urls))
        elif discovery_type == 'all':
            run_async(mcp_registry.discover_kubernetes_services())
            run_async(mcp_registry.discover_http_services([]))
        
        return jsonify({
            "status": "success",
            "message": f"Discovery triggered for {discovery_type}"
        })
    except Exception as e:
        logger.error(f"Registry discover error: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/mcp/registry/reload', methods=['POST'])
def registry_reload():
    """Reload registry configuration"""
    try:
        run_async(mcp_registry.reload_configuration())
        return jsonify({
            "status": "success",
            "message": "Registry configuration reloaded"
        })
    except Exception as e:
        logger.error(f"Registry reload error: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/mcp/registry/health')
def registry_health():
    ...
    try:
        ...
        logger.error(f"Registry health error: {e}")
        return jsonify({"error": str(e)}), 500

# Enhanced MCP Client Routes
@app.route('/mcp/status')
def mcp_status():
    """Get enhanced MCP client status"""
    try:
        status = enhanced_mcp_manager.get_connection_status()
        connected_servers = enhanced_mcp_manager.get_connected_servers()
        servers_info = enhanced_mcp_manager.get_all_servers_info()
        stats = enhanced_mcp_manager.get_registry_stats()
...
def mcp_connect():
    ...
    try:
        ...
        success = run_async(enhanced_mcp_manager.connect_to_server(server_name))
...
def mcp_disconnect():
    ...
    try:
        ...
        success = run_async(enhanced_mcp_manager.disconnect_server(server_name))
...
@app.route('/mcp/connect-all', methods=['POST'])
def mcp_connect_all():
    ...
    try:
        ...
        # Run async connection to all servers
        connected_servers = run_async(enhanced_mcp_manager.discover_and_connect_servers(tags))
        
        return jsonify({
            "status": "success",
            "message": f"Connected to {len(connected_servers)} servers",
            "connected_servers": connected_servers
        })
    except Exception as e:
        logger.error(f"MCP connect-all error: {e}")
        return jsonify({"error": str(e)}), 500
...
def mcp_tools(server_name):
    ...
    try:
        if server_name not in enhanced_mcp_manager.get_connected_servers():
            return jsonify({"error": f"Server {server_name} not connected"}), 400
        
        # Run async tool listing
        tools = run_async(enhanced_mcp_manager.list_tools(server_name))
...
def mcp_call():
    ...
    try:
        ...
        # Run async tool call
        result = run_async(enhanced_mcp_manager.call_tool(server_name, tool_name, arguments))
...
@app.route('/')
def index():
    """Main playground interface with enhanced MCP registry"""
    ...
    <body>
        <div class="container">
            <h1>🔍 RAG Playground - Enhanced with MCP Registry</h1>
                ...
                <div class="mcp-section">
                    ...
                    <div class="mcp-panel">
                        ...
                        <div class="mcp-actions">
                            ...
                            <button onclick="triggerDiscovery()">Discover Services</button>
                            <button onclick="connectAllServers()">Connect All</button>
                            <button onclick="reloadRegistry()">Reload Registry</button>
                            <button onclick="showRegistryHealth()">Health Check</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script>
            // Enhanced MCP Client JavaScript functions
            async function refreshMCPStatus() {
                try {
                    const response = await fetch('/mcp/status');
                    const data = await response.json();
                    updateMCPStatus(data);
                    updateRegistryStats(data.stats);
                } catch (e) {
            ...
            async function updateRegistryStats() {
                try {
                    const response = await fetch('/mcp/registry/status');
                    const data = await response.json();
                    if (data.status === 'success') {
                        const stats = data.stats;
                        const statsDiv = document.getElementById('registry-stats');
                        statsDiv.innerHTML = `
                            <strong>Registry Stats:</strong><br>
                            Total Servers: ${stats.registry.total_servers} | 
                            Enabled: ${stats.registry.enabled_servers} | 
                            Healthy: ${stats.registry.healthy_servers} | 
                            Connected: ${stats.client.connected_servers}<br>
                            <small>Discovery Methods: ${Object.entries(stats.registry.discovery_methods).map(([k,v]) => `${k}:${v}`).join(', ')}</small>
                        `;
                    }
                } catch (e) {
                    console.error('Failed to update registry stats:', e);
                }
            }
            ...
            function updateServerList(data) {
                ...
                if (data.servers_info) {
                    Object.entries(data.servers_info).forEach(([serverName, serverInfo]) => {
                        ...
                        const healthStatus = serverInfo.health_status;
                        const discoveryMethod = serverInfo.discovery_method;
                        ...
                        serverDiv.innerHTML = `
                            <div>
                                ...
                                <div class="discovery-method">${discoveryMethod}</div>
                                <div class="health-status ${healthStatus}">${healthStatus}</div>
                                ...
                                ${!isEnabled ? ' (Disabled)' : ''}
                                ${serverInfo.tools_count > 0 ? ` | ${serverInfo.tools_count} tools` : ''}
                            </div>
            ...
            async function triggerDiscovery() {
                try {
                    const response = await fetch('/mcp/registry/discover', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                    ...
                    if (data.status === 'success') {
                        alert(`Discovery triggered: ${data.message}`);
                        setTimeout(refreshMCPStatus, 2000); // Refresh after 2 seconds
                    } else {
            ...
            async function connectAllServers() {
                try {
                    const response = await fetch('/mcp/connect-all', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({})
                    });
                    ...
                    if (data.status === 'success') {
                        alert(`Connected to ${data.connected_servers.length} servers: ${data.connected_servers.join(', ')}`);
                        refreshMCPStatus();
                    } else {
                        alert(`Connect all failed: ${data.error}`);
                    }
                } catch (e) {
                    alert(`Connect all error: ${e.message}`);
                }
            }

            async function reloadRegistry() {
                try {
                    const response = await fetch('/mcp/registry/reload', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'}
                    });
                    const data = await response.json();
                    ...
                    if (data.status === 'success') {
                        alert('Registry configuration reloaded');
                        refreshMCPStatus();
                    } else {
            ...
            async function showRegistryHealth() {
                try {
                    const response = await fetch('/mcp/registry/health');
                    const data = await response.json();
                    
                    if (data.status === 'success') {
                        let healthInfo = 'Registry Health Status:\\n\\n';
                        Object.entries(data.health).forEach(([name, health]) => {
                            healthInfo += `${name}:\\n`;
                            healthInfo += `  Status: ${health.status}\\n`;
                            healthInfo += `  Last Check: ${new Date(health.last_check * 1000).toLocaleString()}\\n`;
                            if (health.response_time) {
                                healthInfo += `  Response Time: ${health.response_time.toFixed(3)}s\\n`;
                            }
                            if (health.error_message) {
                                healthInfo += `  Error: ${health.error_message}\\n`;
                            }
                            healthInfo += '\\n';
                        });
                        alert(healthInfo);
...
if __name__ == '__main__':
    logger.info("=== Enhanced RAG Playground with MCP Registry Started ===")
    
    # Check MCP availability
    if MCP_AVAILABLE:
        logger.info("MCP libraries available - Enhanced MCP client functionality enabled")
    else:
    ...
    logger.info("Features: Document search + MCP registry + Service discovery")
