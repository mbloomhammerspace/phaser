...
print("=== Fixing Milvus Schema for RAG Server ===")
...
if utility.has_collection('hammerspace_docs'):
    ...
    if entities > 0:
        # Query all data
        results = old_collection.query(expr="pk >= 0", output_fields=["pk", "vector", "source", "content_metadata", "text"])
...
# LangChain Milvus typically expects these field names:
fields = [
    FieldSchema(name="pk", dtype=DataType.INT64, is_primary=True, auto_id=True),
    FieldSchema(name="vector", dtype=DataType.FLOAT_VECTOR, dim=2048),
    FieldSchema(name="source", dtype=DataType.VARCHAR, max_length=256),
    FieldSchema(name="content", dtype=DataType.VARCHAR, max_length=4096),  # Changed from 'text' to 'content'
]
...
if data:
    ...
    contents = [item['text'] for item in data]  # Map 'text' to 'content'
