...
get_stats() {
    ...
    kubectl logs ingestor-server-84697ff646-ksbtr --tail=10 2>/dev/null | grep -E "(Processing|batch|complete|documents)" | tee -a $LOG_FILE
...
kubectl logs ingestor-server-84697ff646-ksbtr --since=30m 2>/dev/null | grep -E "Time taken:" | tail -20 | tee -a $LOG_FILE
